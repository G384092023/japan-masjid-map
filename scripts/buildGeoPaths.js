...
// Dedicated clear inset projection for Okinawa in the southwest corner
...
geojson.features.forEach(feature => {
  ...
  // Adjust specific labels for maximum legibility in dense areas
  if (meta.id === 'Tokyo') { labelX += 14; labelY -= 2; }
  if (meta.id === 'Osaka') { labelX += 6; labelY += 2; }
  if (meta.id === 'Kanagawa') { labelX += 8; labelY += 10; }
  if (meta.id === 'Saitama') { labelY -= 6; }
  if (meta.id === 'Chiba') { labelX += 12; }
  if (meta.id === 'Kyoto') { labelY -= 14; }
  if (meta.id === 'Aichi') { labelY += 6; }
  if (meta.id === 'Fukuoka') { labelY -= 6; }
  if (meta.id === 'Kagawa') { labelY -= 6; }
  if (meta.id === 'Okinawa') { labelX = 160; labelY = 690; }
...
console.log('Successfully built ' + paths.length + ' real prefecture geographic shapes in src/data/japanRealGeoPaths.js');
